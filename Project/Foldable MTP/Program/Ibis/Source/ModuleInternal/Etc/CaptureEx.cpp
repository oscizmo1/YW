...
//-------------------------------------------------------------------------------
void CaptureEx::SaveBmpToFile(LPTSTR szFileName,int W,int H,int Bpp,int* lpBits)
{
	BITMAPINFO Bmi={0};
...
//-------------------------------------------------------------------------------
void CaptureEx::Get24BitBmp ( const int &nWidth, const int &nHeight,const HBITMAP &hBitmap , BYTE *lpDesBits)
{
	HDC hDC = ::GetDC( 0 );
...
//-------------------------------------------------------------------------------
BOOL CaptureEx::CaptureWindow( HWND hWndSrc,double fPreviewRatio,LPTSTR lpszFileName,BOOL bSaveToFile)
{
	RECT rc={0};
	...
	// 	if (bSaveToFile)
	// 	{
	// 		HBITMAP hCaptureBitmap = ::CreateBitmap(Width,Height,1,Bpp,lpBits1);
	// 		CImage image;
	...
	if (bSaveToFile)
	{
		CString strTemp, strBmp;
		strTemp.Format(_T("%s"), lpszFileName);
		int nLength = strTemp.GetLength();
		int nIndex = strTemp.ReverseFind(_T('.'));
		strTemp.Delete(nIndex, nLength - nIndex);
		strBmp = strTemp + _T(".bmp");
		SaveBmpToFile(strBmp, Width, Height,24, (int*)lpBits2);
		CImage image;
		image.Load(strBmp);
		image.Save(lpszFileName );
		::DeleteFile(strBmp);
...
//-------------------------------------------------------------------------------
void CaptureEx::CaptureDesktop (double fPreviewRatio,LPTSTR lpszFileName,BOOL bSaveToFile)
{
	RECT rc;
...
//-------------------------------------------------------------------------------
void CaptureEx::StretchDIBitsToHwnd( HWND hWnd,double fRatio,int Width,int Height,int Bpp,BYTE *lpBits)
{
	HDC hDC = ::GetDC(hWnd);
	BITMAPINFO bmp={0};
    ...
    bmp.bmiHeader.biSizeImage = bmp.bmiHeader.biWidth*bmp.bmiHeader.biHeight*Bpp/8;

	double fScaleWidth	= Width*fRatio;
	double fScaleHeight	= Height*fRatio;

	::SetWindowPos(hWnd,0,0,0,(int)fScaleWidth,(int)fScaleHeight,SWP_NOMOVE|SWP_NOZORDER);
	::StretchDIBits(hDC,0,0,(int)fScaleWidth,(int)fScaleHeight,0,0,Width,Height,lpBits, &bmp,DIB_RGB_COLORS,SRCCOPY);	
...
BOOL CaptureEx::Capture( HWND hwnd,HDC memDC)
{
	...
	{
		::AfxMessageBox(_T("cant gain address of PrintWindow(..) api\nplease update your sdk"));
		Ret = FALSE;
	}
	::FreeLibrary(handle);
...
//-------------------------------------------------------------------------------
void CaptureEx::UpdateLastImage( int Width, int Height, int Size,int Bpp, BYTE* lpBits)
{
	if ( m_pLastImage )
	{
		delete []m_pLastImage;
...
//-------------------------------------------------------------------------------
void CaptureEx::SaveLastCaptured(LPTSTR szFileName)
{
	if ( m_pLastImage )
