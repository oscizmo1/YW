...
int CStateBZone::Run()
{
	...
	case stepStart:
		...
		if(IsReturnOk())
		{
			...
			if(theProcBank.m_bIsSetZone[m_Shuttle][ZONE_ID_B] == FALSE)
			{
				...
				MCRtoBZoneSetTimeEnd(m_Shuttle);
				BZone_SetTimeStart(m_Shuttle);
				CellLog_BZoneMTPReadyTime_SetStartTime(m_Shuttle);
	...
	case stepZoneCCheck:
		...
		if(IsReturnOk())
		{
			...
			CellLog_BZoneMTPReadyTime_SetEndTime(m_Shuttle);
			m_Timer.Start();
			nStep++;
		}
		else if(m_Timer.Stop(FALSE) > 5.)
	...
	case stepBefStart:
		...
		if(bNext)
		{
			m_pCurrentModule = theInspModuleBank.GetNextModule_BZone_Bef(m_strCurrentModuleName, m_Shuttle);
			if(m_pCurrentModule)
			{
	...
	case stepMustStart:
		...
		if(bNext)
		{
			m_pCurrentModule = theInspModuleBank.GetNextModule_BZone_Must(m_strCurrentModuleName, m_Shuttle);
			if(m_pCurrentModule)
			{
			...
			else
			{
				...
				CellLog_BZoneETCTime_SetStartTime(m_Shuttle);
				nStep++;
			}
		}
		break;
	...
	case stepAftStart:
		...
		if(bNext)
		{
			m_pCurrentModule = theInspModuleBank.GetNextModule_BZone_Aft(m_strCurrentModuleName, m_Shuttle);
			if(m_pCurrentModule)
			{
			...
			else
			{
				...
				CellLog_BZoneETCTime_SetEndTime(m_Shuttle);
				BZone_SetTimeEnd(m_Shuttle);
				m_Timer.Start();
				nStep++;
			}
		}
