...
BOOL CFileSupport::CopyDirectory( LPCTSTR sourcePath, LPCTSTR targetPath, BOOL bSubDirCopy /*= FALSE */ )
{
	...
	{
		AfxMessageBox("디렉토리 오류, 재설정...", MB_OK|MB_ICONERROR|MB_SYSTEMMODAL);
		return FALSE;
	}
	if (nResult == 0 && FileOp.fAnyOperationsAborted == TRUE) {
		AfxMessageBox("사용자에 의해 동작이 중단되었습니다!");
...
{
	fullPath.Replace("\\", "/");
	return fullPath.Left(fullPath.ReverseFind('/')+1);
}
...
// 		return;
	...
	CString searchPath;
	searchPath.Format("%s/*.*", lpPathName);
	BOOL bWorking = finder.FindFile(searchPath);
	while (bWorking){
		bWorking = finder.FindNextFile();
		strModelname = finder.GetFileName();
		strFullPath.Format("%s/%s", lpPathName, strModelname);
...
//				}
	...
	if (!::RemoveDirectory(lpPathName)) {
		...
		MessageBox( NULL, (LPCTSTR)errMsg, "Error", MB_OK | MB_ICONINFORMATION );
		// Free the buffer.
		LocalFree( lpMsgBuf );
	}
}
...
void CFileSupport::MoveFile(LPCTSTR lpExistingFileName, LPCTSTR lpNewFileName)
{
...
void CFileSupport::WriteSystemLog( LPCTSTR lpLogString )
{
	// 나중에 필요시 사용할것
	// 파일 삭제시 C:\SystemLog\에 txt파일로 삭제 경로 기록됨
// 	LPCTSTR lpDirectoryPath = "C:\\SystemLog";
// 	if (!DirectoryCheck(lpDirectoryPath))
// 	{
// 		if(!CreateDirectory(lpDirectoryPath, NULL))
// 			return;
// 	}
// 	char buffer[250];
// 	sprintf_s(buffer, "%s\\%s.Log", lpDirectoryPath, CTime::GetCurrentTime().Format("%Y-%m-%d"));
// 	FILE* m_logfp;
// 	if(fopen_s(&m_logfp, (LPCTSTR)buffer, "at+") == 0)
// 	{
// 		fprintf(m_logfp, "%s\tDELETE\t%s\n", CTime::GetCurrentTime().Format("%H:%M:%S"), lpLogString);
// 		fclose(m_logfp);
// 	}
// 	m_logfp = NULL;
}